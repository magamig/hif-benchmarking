...
class VSR_CAS(torch.nn.Module):
    ...
    """
    def __init__(self , channel0 ,factor,P ):
        ...
        self.fe_conv9 = torch.nn.Conv2d(in_channels=576, out_channels=64, kernel_size=3, padding=3 // 2)
        self.fe_conv10= torch.nn.Conv2d(in_channels=192, out_channels=64, kernel_size=3, padding=3 // 2)
        # self.conv_downsample = torch.nn.Conv2d(in_channels=31, out_channels=31, kernel_size=13,stride=8, padding=13 // 2)
        self.conv_downsample = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels=31, out_channels=31, kernel_size=5, stride=2,
                            padding=5 // 2),
            torch.nn.ReLU(),
            torch.nn.Conv2d(in_channels=31, out_channels=31, kernel_size=5, stride=2,
                                               padding=5 // 2),
        ...
        self.conv_upsample1 = torch.nn.ConvTranspose2d(in_channels=31, out_channels=31, kernel_size=5, stride=2,
                                                      padding=5 // 2)
        self.conv_upsample2 = torch.nn.ConvTranspose2d(in_channels=31, out_channels=31, kernel_size=5, stride=2,
                                                       padding=5 // 2)
        self.conv_upsample3 = torch.nn.ConvTranspose2d(in_channels=31, out_channels=31, kernel_size=5, stride=2,
                                                       padding=5 // 2)
    ...
    def recon(self, features, recon,LR, RGB,id_layer):
        ...
        err1 = self.conv_upsample3(
            self.acti(self.conv_upsample2(
                self.acti(
                    self.conv_upsample1(down - LR , output_size=[int(sz[0]),int(sz[1]),int(sz[2]/4),int(sz[3]/4)] )
                ),output_size=[int(sz[0]),int(sz[1]),int(sz[2]/2),int(sz[3]/2)]
            )),output_size=sz
        )
    ...
    def forward(self, LR,RGB): # [batch_size ,3 ,7 ,270 ,480] ;
        ...
        conv_out = conv_out+z
        x = self.recon(conv_out, x, y, RGB,  id_layer=1)
        ...
        conv_out = conv_out+z
        x = self.recon(conv_out, x, y, RGB,  id_layer=2)
        ...
        v=v+z
        z = self.recon_noisy(z, x, v, RGB, 0)
        conv_out,fe7 = self.spatial(self.fe_conv8(torch.cat((self.fe_conv1(z), fe6),1)))
        conv_out=conv_out+z
        x = self.recon(conv_out, x, y, RGB, id_layer=3)

        z = x
        v, fe8 = self.spatial(self.fe_conv9(torch.cat((self.fe_conv1(z), fe, fe2, fe4,fe6), 1)))
        v = v + z
        z = self.recon_noisy(z, x, v, RGB, 0)
        conv_out, _ = self.spatial(self.fe_conv10(torch.cat((self.fe_conv1(z), fe8), 1)))
        conv_out = conv_out + z
